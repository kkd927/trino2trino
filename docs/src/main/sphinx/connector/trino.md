...
- Remote delegation probes ``CHAR`` to ``VARCHAR`` cast semantics and delegates
  these casts only when the remote retains the padding expected by Trino 479
...
smoke test for the production shape where a small federated Trino 479 cluster
queries a separate Trino 479 cluster with a Delta Lake catalog. It reuses the
``target/trino-trino-479`` package produced by ``mvn -B clean verify`` and is
