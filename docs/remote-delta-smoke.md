...
contract with in-process Trino 479 query runners.
...
federated Trino 479 cluster queries a separate Trino 479 cluster whose remote
...
- `trino-local-delta`: Trino 479 with the `trino2trino` plugin mounted from
  `target/trino-trino-479`
- `trino-remote-delta`: Trino 479 with a native `delta_lake` catalog
...
- The default CI path runs this smoke test after `mvn -B clean verify` and uses
  the `target/trino-trino-479` package created by that build.
