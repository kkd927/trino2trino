...
contract with in-process Trino 477 query runners.
...
federated Trino 477 cluster queries a separate Trino 477 cluster whose remote
...
- `trino-local-delta`: Trino 477 with the `trino2trino` plugin mounted from
  `target/trino-trino-477`
- `trino-remote-delta`: Trino 477 with a native `delta_lake` catalog
...
- The default CI path runs this smoke test after `mvn -B clean verify` and uses
  the `target/trino-trino-477` package created by that build.
