...
| 2 | 조인 푸시다운 시 transport 변환(CAST 래핑)이 조인 소스와 최종 스캔에서 **두 번** 적용 → JSON transport 컬럼이면 원격 에러 추정 | TrinoClient.java:406-425, 690-718 | transport projection을 **스캔 시점(`split` 존재 시)에만** 적용. 전제(플래닝=split 없음, 스캔=split 있음)는 base-jdbc 448 소스로 확인 완료 |
...
- 축소된 베이스 오버라이드 복원 — Trino 448에는 `newTrinoTable` 원격 fixture 훅이 없다.
  두 인자 `createAggregationTestTable`은 존재하지만 `onRemoteDatabase()`를 사용하는 숫자
  aggregation 전용 helper이므로 그대로 상속한다. read-only 커넥터의 string/char
  case-sensitive aggregation fixture는 `createRemoteFixtures()`에서 원격에 미리 만들고
  target 전용 `testCaseSensitiveAggregationPushdown` override로 검증한다. join override는
  EAGER 전략과 `complex_join_pushdown_enabled` 양쪽 계획을 검증하기 위해 유지한다.
  `testArithmeticPredicatePushdown`은 plan 검증을 추가하고, 남는 오버라이드에는 사유 주석
...
coalesce delegation 실지원(Trino 448 번역기에는 `visitCoalesce` 자체가 없어 connector-only
변경으로는 불가하며 target 업그레이드 또는 upstream 지원이 선행되어야 함).
...
- base-jdbc 448 소스 확인: 공개 `prepareQuery`(조인 소스 합성)는 split=empty,
...
- trino-main 448 connector-expression 번역기 소스 확인: Coalesce와 If/Case/Try는 visit 메서드 자체가
  없으며, `$array` 방출 지점은
  `visitIn`(IN-리스트 포장) 하나뿐
- base-jdbc 448 `DefaultQueryBuilder` 소스 확인: 컬럼 mapping write function은 튜플 도메인
...
- `NumberType`/`TrinoNumber`는 Trino 448 SPI에 존재하지 않으며 480에서 처음 추가됨 —
  448 포트에서는 관련 codec·mapping·binding·test를 모두 제외
