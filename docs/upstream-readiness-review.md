...
| 2 | 조인 푸시다운 시 transport 변환(CAST 래핑)이 조인 소스와 최종 스캔에서 **두 번** 적용 → JSON transport 컬럼이면 원격 에러 추정 | TrinoClient.java:406-425, 690-718 | transport projection을 **스캔 시점(`split` 존재 시)에만** 적용. 전제(플래닝=split 없음, 스캔=split 있음)는 base-jdbc 479 바이트코드로 확인 완료 |
...
coalesce delegation 실지원(Trino 479 번역기에는 `visitCoalesce` 자체가 없어 connector-only
...
- base-jdbc 479 바이트코드 확인: 공개 `prepareQuery`(조인 소스 합성)는 split=empty,
...
- trino-main 479 번역기 바이트코드 확인: Coalesce와 If/Case/Try는 visit 메서드 자체가
  없으며, `$array` 방출 지점은
  `visitIn`(IN-리스트 포장) 하나뿐
- base-jdbc 479 `DefaultQueryBuilder` 바이트코드 확인: 컬럼 mapping write function은 튜플 도메인
...
- `NumberType`/`TrinoNumber`는 Trino 479 SPI에 존재하지 않으며 480에서 처음 추가됨 —
  479 포트에서는 관련 codec·mapping·binding·test를 모두 제외
