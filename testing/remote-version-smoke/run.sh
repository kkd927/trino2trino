...
run_version() (
  ...
  if [[ "${local_version}" -le 448 ]]; then
    # Trino 448 has no JDBC projection hook. Require the timestamp predicates
    # to reach the remote scan while date_diff remains a correct local project.
    assert_output_contains "ScanProject[table = remote_memory:" "TSTZ target-compatible projection plan" "${tstz_binding_output}"
    assert_output_contains "constraint on [p3_region, p12_region]" "TSTZ delegated predicate plan" "${tstz_binding_output}"
    assert_output_contains "expr_0 := date_diff" "TSTZ local projection plan" "${tstz_binding_output}"
  else
