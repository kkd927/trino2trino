...
# Trino 448 has no JDBC projection hook: keep regexp_extract local while
# requiring the regexp predicate to be delegated as a remote constraint.
...
assert_query_contains "ScanProject[table = remote_delta:" "${regexp_filter_plan_sql}"
assert_query_contains "expr := regexp_extract" "${regexp_filter_plan_sql}"
assert_query_contains "constraints=[ParameterizedExpression[expression=regexp_like" "${regexp_filter_plan_sql}"
