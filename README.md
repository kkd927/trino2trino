...
- `CALL system.execute(...)` is not available in Trino 448; the only inherited
  base-JDBC procedure is `CALL system.flush_metadata_cache()`, which clears
  local connector metadata caches
